...
Perfect block requires an accepted false-to-true Block transition within .12 seconds before an otherwise blockable frontal hit. It prevents chip, hit/knockback and incoming-hit telemetry, adds 40 base style points at the current multiplier plus one multiplier level, and staggers the attacker for .6 seconds. The captured enemy serial/token/grab/projectile is canceled; the resolver rechecks validity before any further player volume, impact visual, followup warning or movement bookkeeping. One block hold can earn one perfect block. Fresh presses need at least .35 seconds since the previous fresh press to arm another window; pressing early remains ordinary block and restarts that rearm interval. Repeated held=true requests never refresh the window. Release cancellation bypasses throttling. These are explicit anti-toggle tuning choices, with actual timing tests separate from the pure policy.
...
World visuals use six native welded cosmetic parts for the bounty pouch/shoulder marker and nine anchored parts for an amber score bead/white orbit ring. They add no queryable damage geometry or touch sensors; the pickup service owns its separate sensor and cleanup. Occluded labels use a 200-stud view distance to cover the authored camera range. Independent source reviews passed; root's geometry fixture must separately verify seven existing queryable rig parts, six existing motors, idempotent bounty marking and noninteractive orb visuals. Physical visibility/polish, actual contact, real intent remotes, multi-volume cancellation and reward integration require their scoped runtime evidence before this work order can close.
