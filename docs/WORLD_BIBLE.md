...
## Authored enemy entry anchors (WO-2.5 source)

`NightfallCity.EnemyEntries` contains one `Stage{n}_Wave{n}` folder for each configured encounter, with `Stage`, `Wave` and `WaveCenterX` attributes. Each folder has four invisible, anchored, non-colliding, non-touching and non-queryable parts: `Left`, `Right`, `Door`, `Drop`; each repeats its name in `EntryKind`. Schema version is 1. No script, imported asset or destruction tag is added.

Side anchors sit 24 studs before/after the encounter center, at Z=-7/+7, clamped six studs inside the district. The background door anchor uses the encounter center at Z=-12. Drop begins eight studs ahead at Y=14, Z=-7 and carries a `LandingPosition` attribute at Y=0. All other marker Y coordinates are zero: the server spawn routine adds the rig root height. Markers are preferred staging anchors; server entry logic may adjust side positions relative to the party and walking limit to preserve a behind-party entrance safely.

Each encounter also receives a five-part original service-door recess, two jambs, lintel and small cyan/amber indicator under `EnemyEntryScenery`. All scenery stays behind the Z=-14 combat boundary, does not obstruct movement or hit queries, and has no floor effect resembling an attack warning. These are entrance cues, not interactable doors or route branches. Existing world/story and controlled route remain intact.

This section records authored source and its schema only. Pulse budgets, placement selection and entry grace belong to the combat owner. The agreed runtime rule is at least 0.6 seconds before attacks, plus grounding before a Drop entrant can attack. Root must run `tests/EnemyEntries.spec.lua` on a rebuilt world, then verify actual pulse/entry behavior and visible composition; the presence of markers alone does not prove those gameplay gates.
