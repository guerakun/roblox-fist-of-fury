...
The launch program is still in progress on `launch-program`. M0 is closed. M1 original heroes/story and source IP checks are implemented; rendered-art review and owner approval of Rook Calder, Bo Marlowe and Isla Veyra remain open. M5 hub, party, admission and return code is implemented with policy and Studio multiplayer evidence. Published travel, live persistence and same-account reconnect remain unverified or blocked by the overnight restrictions.

M2 source is frozen at `9f29012`; both place files were rebuilt from that commit. The first final HumanBot run cleared in **430.96 s**, losing **4 stocks**, taking **991 damage**, with **8.42% idle**, **100% qualified flank windows**, **0 cap breaches**, and **80 audited hit receipts / 0 outside / 0 invalid viewports**. This is one run, not the five-run aggregate. Baseline was 5/5 clears, mean **278.27 s / 0.80 stocks / 543 damage**, pooled idle **16.10%**, flank **45.45%**. Historical crowd failures remain saved; final footwork crowd/CPU rerun is pending.

M6 includes an owner checklist, analytics observer, original icon concept and hub resource fixes. Two-client hub regression passed 8 assertions in 26.24 s. Input focus/modal cleanup is queued after the bot freeze; the movement-origin security gap remains a documented proposal, not a fix. **M3 and M4 have not started; all work orders remain in the table below.**

**Exact next:** root continues seed **1102**, then **1103–1105**, on the unchanged M2 build; collect final crowd/CPU evidence, save the M2 engineering checkpoint, then begin M3. Human/device acceptance remains an owner gate even after engineering progresses. No publication, Creator Dashboard, sales, live DataStores or place settings were changed.

All notes are indexed in [launch/README](launch/README.md). Source audits preserve the three input cleanup findings and the proposed movement-authority design. The post-run scene query is recorded with its test-script and rendering limitations; it does not certify device performance. Seed1102: Victory,404.08s,3stocks,946damage,idle10.40%,flank96.00%,cap0;frustum79audited/0outside/0invalid. This is an individual result; five-run aggregate pending. Seed1103: Victory,382.18s,2stocks,683damage,idle11.27%,flank95.45%,cap0;frustum59audited/0outside/0invalid. This is an individual result; five-run aggregate pending.
