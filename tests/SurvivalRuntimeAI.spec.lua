...
return function()
    ...
    local ok,err=xpcall(function()
        ...
        C.EnterDistrict(Vector3.new(90,4,0))
        assert(C.GetSnapshot(a).downed and C.GetSnapshot(a).stocks==0 and C.GetSnapshot(a).downedRemaining==0,"Zero-stock travel revived or renewed deadline")
        local beforeInvalid=C.GetSnapshot(b)
        assert(not C.CompleteDistrict("old-campaign",1)and not C.CompleteDistrict("survival-fixture",4)and not C.CompleteDistrict("survival-fixture",1.5),"Invalid clear accepted")
        assert(C.GetSnapshot(b).stocks==beforeInvalid.stocks and C.GetSnapshot(b).percent==beforeInvalid.percent,"Invalid clear mutated survival")
        ...
        result={passed=true,channel=true,throttledRelease=true,movementCancel=true,hitCancel=true,expiry=true,clearIdempotent=true,staleClearRejected=true,zeroStockTravel=true,travelPersists=true,retryTwo=true,actualReviveRemote=true,physicalInputs=false}
    end,debug.traceback)
