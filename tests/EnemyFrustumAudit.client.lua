...
local report={schema=1,measurement="attacker position projected through victim camera at Hit receipt",hits=0,outside=0,events={},fixture=false}
...
fx.OnClientEvent:Connect(function(event)
    ...
    if #report.events<1000 then table.insert(report.events,{visible=visible,x=point.X,y=point.Y,depth=point.Z,serverTime=event.serverTime,received=workspace:GetServerTimeNow()})end
    publish()
end)
